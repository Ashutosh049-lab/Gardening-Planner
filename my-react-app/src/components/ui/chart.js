...
const ChartTooltipContent = React.forwardRef(({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, }, ref) => {
    ...
    return (_jsxs("div", { ref: ref, className: cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className), children: [tooltipLabel ? (_jsx("div", { className: cn("font-medium", labelClassName), children: tooltipLabel })) : null, _jsx("div", { className: "grid gap-1.5", children: payload.map((item, index) => {
                    const key = `${nameKey || item.name || item.dataKey || "value"}`;
                    const itemConfig = item.payload?.[key];
                    const indicatorColor = color || item.payload?.fill || item.color;
                    return (_jsx("div", { className: cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center"), children: formatter && item?.value !== undefined && item.name ? (formatter(item.value, item.name, item, index, payload)) : (_jsxs(_Fragment, { children: [itemConfig?.icon ? (_jsx(itemConfig.icon, {})) : (!hideIndicator && (_jsx("div", { className: cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", {
                                        "h-2.5 w-2.5": indicator === "dot",
                                        "w-1": indicator === "line",
                                        "w-0 border-[1.5px] border-dashed bg-transparent": indicator === "dashed",
                                        "my-0.5": indicator === "dashed",
                                        ...
                                        "--color-bg": indicatorColor,
                                        "--color-border": indicatorColor,
                                    } }))), _jsxs("div", { className: cn("flex flex-1 justify-between leading-none", hideIndicator ? "items-end" : "items-center"), children: [_jsx("div", { className: "grid gap-1.5", children: _jsx("span", { className: "text-muted-foreground", children: itemConfig?.label || item.name }) }), item.value && (_jsx("span", { className: "font-mono font-medium tabular-nums text-foreground", children: typeof item.value === 'number' ? item.value.toLocaleString() : item.value }))] })] })) }, item.dataKey || index));
                }) })] }));
});
ChartTooltipContent.displayName = "ChartTooltipContent";
// Chart legend component
...
const ChartLegendContent = React.forwardRef(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
    ...
    return (_jsx("div", { ref: ref, className: cn("flex items-center justify-center gap-4", className), children: payload.map((item, index) => {
            const key = `${nameKey || item.value || item.dataKey || "value"}`;
            const itemConfig = item.payload?.[key];
            return (_jsxs("div", { className: cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground"), children: [itemConfig?.icon && !hideIcon ? (_jsx(itemConfig.icon, {})) : (!hideIcon && (_jsx("div", { className: "h-2 w-2 shrink-0 rounded-[2px]", style: {
                            backgroundColor: item.color,
                        } }))), itemConfig?.label || item.value] }, item.value || index));
        }) }));
});
ChartLegendContent.displayName = "ChartLegendContent";
