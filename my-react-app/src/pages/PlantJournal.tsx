...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { format } from 'date-fns';
import { Plus, Camera, Droplets, Scissors, Sprout, Calendar, BookOpen, Upload, X } from 'lucide-react';
import { useDropzone } from 'react-dropzone';
...
export default function PlantJournal() {
  ...
  const getCareIcon = (action: string) => {
    switch (action) {
      ...
      case 'repotting': return <Calendar className="h-5 w-5 text-purple-500" />;
      default: return <Calendar className="h-5 w-5 text-gray-500" />;
    }
  };

  const getCareActionEmoji = (action: string) => {
    switch (action) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-gray-600">Loading journal...</p>
          </div>
        </div>
      </div>
    );
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[600px] bg-white max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-3">
                  <Label htmlFor="careAction" className="text-base font-medium text-gray-700">Care Action</Label>
                  <Select onValueChange={(value) => setValue('careAction', value)}>
                    <SelectTrigger className="h-12 bg-gray-50 border-2 border-gray-200">
                      <SelectValue placeholder="🔧 Select care action (optional)" />
                    </SelectTrigger>
                ...
                <div className="space-y-3">
                  <Label htmlFor="text" className="text-base font-medium text-gray-700">Notes *</Label>
                  <Textarea
                    id="text"
                    placeholder="What did you observe or do with your plant today? Describe growth, health, care activities..."
                    className="min-h-[120px] bg-gray-50 border-2 border-gray-200"
                    {...register('text', { required: 'Notes are required' })}
                  />
                  {errors.text && (
                    <p className="text-sm text-red-500 bg-red-50 p-2 rounded">{errors.text.message}</p>
                  )}
                </div>

                <div className="space-y-3">
                  <Label htmlFor="tags" className="text-base font-medium text-gray-700">Tags</Label>
                  <Input
                    id="tags"
                    placeholder="growth, flowering, pest-control (comma separated)"
                ...
                {/* Image Upload */}
                <div className="space-y-3">
                  <Label className="text-base font-medium text-gray-700">Photos</Label>
                  <div
                    {...getRootProps()}
                  ...
                  >
                    ...
                    <p className="text-base font-medium text-gray-700 mb-1">
                      {isDragActive ? 'Drop photos here...' : 'Add photos to your entry'}
                    </p>
        ...
        {entries.length === 0 ? (
          <Card className="bg-white shadow-lg border-0">
            <CardContent className="text-center py-16">
              ...
              <h3 className="text-2xl font-semibold text-gray-700 mb-3">No journal entries yet</h3>
          ...
          <div className="space-y-6">
            {entries.map((entry) => (
              <Card key={entry.id} className="bg-white shadow-lg hover:shadow-xl transition-all duration-300 border-0 overflow-hidden">
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-4">
                      <span className="text-3xl">{getPlantEmoji(entry.plantId)}</span>
                      <div>
                        <h3 className="text-xl font-semibold text-gray-800">{getPlantName(entry.plantId)}</h3>
                        <p className="text-sm text-gray-500">
                          {format(entry.createdAt, 'MMM d, yyyy • h:mm a')}
                        </p>
                      </div>
                    ...
                    {entry.careAction && (
                      <div className="flex items-center space-x-2">
                        <span className="text-2xl">{getCareActionEmoji(entry.careAction)}</span>
