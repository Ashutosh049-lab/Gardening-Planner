...
export default function Reminders() {
  ...
  const getCareIcon = (type: string) => {
    switch (type) {
      ...
      case 'pruning': return <Scissors className="h-5 w-5 text-orange-500" />;
      default: return <Calendar className="h-5 w-5 text-gray-500" />;
    }
  };

  const getCareEmoji = (type: string) => {
    switch (type) {
  ...
  const getReminderStatus = (reminder: Reminder) => {
    if (!reminder.enabled) return { text: 'Disabled', color: 'bg-gray-100 text-gray-800 border-gray-200' };
    if (isPast(reminder.nextRunAt) && !isToday(reminder.nextRunAt)) return { text: 'Overdue', color: 'bg-red-100 text-red-800 border-red-200' };
    if (isToday(reminder.nextRunAt)) return { text: 'Due Today', color: 'bg-yellow-100 text-yellow-800 border-yellow-200' };
    if (isTomorrow(reminder.nextRunAt)) return { text: 'Due Tomorrow', color: 'bg-blue-100 text-blue-800 border-blue-200' };
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-gray-600">Loading reminders...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[500px] bg-white">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="space-y-3">
                  <Label htmlFor="plantId" className="text-base font-medium text-gray-700">Select Plant *</Label>
                  <Select onValueChange={(value) => setValue('plantId', value)}>
                ...
                <div className="space-y-3">
                  <Label htmlFor="type" className="text-base font-medium text-gray-700">Care Type *</Label>
                  <Select onValueChange={(value) => setValue('type', value as 'watering' | 'fertilizing' | 'pruning')}>
                    <SelectTrigger className="h-12 bg-gray-50 border-2 border-gray-200">
                      <SelectValue placeholder="🔧 Select care type" />
                    </SelectTrigger>
                  ...
                  {errors.type && (
                    <p className="text-sm text-red-500 bg-red-50 p-2 rounded">Please select a care type</p>
                  )}
                </div>

                <div className="space-y-3">
                  <Label htmlFor="ruleDays" className="text-base font-medium text-gray-700">Repeat Every (days) *</Label>
                  <Input
                    id="ruleDays"
                    type="number"
                    min="1"
                  ...
                  />
                  {errors.ruleDays && (
                    <p className="text-sm text-red-500 bg-red-50 p-2 rounded">{errors.ruleDays.message}</p>
                  )}
                  <p className="text-sm text-gray-500">
                    Common schedules: Watering (2-3 days), Fertilizing (14-30 days), Pruning (30-60 days)
                  </p>
                </div>
          ...
          <CardContent className="pt-6">
            {upcomingReminders.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-lg font-medium text-gray-700 mb-2">No tasks due today! 🎉</p>
                <p className="text-gray-500">Your plants are all caught up on their care schedule.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {upcomingReminders.map((reminder) => {
                  const status = getReminderStatus(reminder);
                  return (
                    <div key={reminder.id} className="flex items-center justify-between p-4 rounded-xl border-2 border-gray-100 bg-white hover:border-green-300 transition-all shadow-sm">
                      <div className="flex items-center space-x-4">
                        <span className="text-3xl">{getCareEmoji(reminder.type)}</span>
                        <span className="text-2xl">{getPlantEmoji(reminder.plantId)}</span>
                        <div>
                          <p className="text-lg font-semibold text-gray-800">{getPlantName(reminder.plantId)}</p>
        ...
        <Card className="bg-white shadow-lg border-0 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-green-50 to-blue-50 border-b">
            <CardTitle className="text-green-700">All Reminders</CardTitle>
          ...
          <CardContent className="pt-6">
            {allReminders.length === 0 ? (
              <div className="text-center py-16">
                <BellOff className="mx-auto h-20 w-20 text-gray-400 mb-6" />
                <h3 className="text-2xl font-semibold text-gray-700 mb-3">No reminders set</h3>
              ...
              <div className="space-y-4">
                {allReminders.map((reminder) => {
                  const status = getReminderStatus(reminder);
                  return (
                    <div key={reminder.id} className="flex items-center justify-between p-4 rounded-xl border-2 border-gray-100 hover:border-green-300 transition-all">
                      <div className="flex items-center space-x-4">
                        <span className="text-2xl">{getCareEmoji(reminder.type)}</span>
                        <span className="text-2xl">{getPlantEmoji(reminder.plantId)}</span>
                        <div>
                          <p className="text-lg font-semibold text-gray-800">{getPlantName(reminder.plantId)}</p>
                          <p className="text-sm text-gray-600 capitalize">
